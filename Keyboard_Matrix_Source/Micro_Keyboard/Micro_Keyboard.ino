...
void setup() {
  TXLED1; RXLED1; // macros to turn TX/RX LEDs off on Pro Micro
...
void loop() {
  ...
  if (key != NO_KEY) {
    ...
    if (key == 21) {if (SYMBLOCK == 1){Keyboard.write('0');}else{
      if (bShiftPressed == 0) {         // if we're not currently pressing Shift, press it
        Keyboard.press(KEY_LEFT_SHIFT); 
        bShiftPressed = 1;              // tell code that Shift is being pressed
      } else if (bShiftPressed == 1){   // if we're currently pressing Shift, release it
        Keyboard.release(KEY_LEFT_SHIFT);
        bShiftPressed = 0;              // tell code that Shift has been released
      }
      if (bShiftDebug == 1){ReportShiftState();}  // if we're debugging Shift, report its state via serial
      bIgnoreShiftState = 1;  // temporarily stop caring about ShiftState -- this will allow Shift to be held down until after the next key is pressed
      }
    }
    ...
    if (key == 29) {Keyboard.press(KEY_RETURN);Keyboard.release(KEY_RETURN);}             //Enter Key 
    if (key == 30) {if (SYMBLOCK == 1){Keyboard.write('$');}else{Keyboard.write('=');}}

    if (key == 31) {Keyboard.write(KEY_LEFT_CTRL); Keyboard.release(KEY_LEFT_CTRL);}      //CTRL Key
    if (key == 32) {Keyboard.write(KEY_LEFT_ALT);Keyboard.release(KEY_LEFT_ALT);}         //Alt Key
    if (key == 33) {if (CAPSLOCK == 0){CAPSLOCK = 1;}else{CAPSLOCK = 0;}                  //Caps Lock Key
                    Keyboard.press(KEY_CAPS_LOCK);Keyboard.release(KEY_CAPS_LOCK);}
    if (key == 34) {if (SYMBLOCK == 0){SYMBLOCK = 1;}else{SYMBLOCK = 0;}}
    if (key == 35) {Keyboard.press(KEY_DELETE); Keyboard.release(KEY_DELETE);}            //Delete Key
    if (key == 36) {Keyboard.write(' ');}
    if (key == 37) {Keyboard.press(KEY_LEFT_ARROW); Keyboard.release(KEY_LEFT_ARROW);}    //Left Direction Key
    if (key == 38) {Keyboard.press(KEY_UP_ARROW); Keyboard.release(KEY_UP_ARROW);}        //Up Direction Key
    if (key == 39) {Keyboard.press(KEY_RIGHT_ARROW); Keyboard.release(KEY_RIGHT_ARROW);}  //Right Direction Key
    if (key == 40) {Keyboard.press(KEY_DOWN_ARROW); Keyboard.release(KEY_DOWN_ARROW);}    //Down Direction Key

    if (bShiftPressed == 1 && bIgnoreShiftState == 0 ){ // if Shift key is being pressed, and we care
        Keyboard.release(KEY_LEFT_SHIFT);   // release Shift key
        bShiftPressed = 0;  // tell code that Shift has been released
        if (bShiftDebug == 1){ReportShiftState();}  // if we're debugging Shift, report its state via serial
    }
      bIgnoreShiftState = 0;  // start caring about ShiftState again -- this will allow Shift to be released after the next key is pressed 
  }
}

void ReportShiftState() {
  Serial.print("bShiftPressed = ");
  Serial.println(bShiftPressed);
}